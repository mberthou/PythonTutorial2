# -*- coding: utf-8 -*-
"""
Created on Tue Nov 26 14:31:34 2019
...
def plotStats(iDirPath,iMeasName, xrange, yrange, devList):
    listVnom = []
    listRnom = []
    listIdeal = []
    listPhiB = []
    ...
    fig.suptitle(devList[0]+" statistics")
    axis[0,0].hist(listVnom,bins='auto')
    axis[0,0].set_xlabel("Vnom (V)")
    axis[0,1].hist(listRnom,bins='auto')
    axis[0,1].set_xlabel("Rnom (V)")
    axis[1,0].hist(listIdeal,bins='auto')
    axis[1,0].set_xlabel("Ideal Factor")
    axis[1,1].hist(listPhiB,bins='auto')
    axis[1,1].set_xlabel("PhiB (eV)")
    
plotStats("C:/Users/Maxime/Documents/Python Scripts/testData",
     "IVDirect",range(-10,100),range(-10,100),["D01"])